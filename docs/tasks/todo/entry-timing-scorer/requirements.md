...
| 直近・次回決算日（earnings risk フラグ用） | `calendars.py --type earnings --start <TODAY> --end <TODAY+45d>`（`obb.equity.calendar.earnings`、nasdaq keyless） | **2026-04-29 初回確認**: nasdaq provider keyless、出力 `{report_date, symbol, eps_consensus, eps_previous, num_estimates, reporting_time, market_cap}`（`shared/openbb/skills/calendars/SKILL.md`）。**2026-04-30 再検証で判明**: nasdaq は窓 ≥ ~56 日で `AttributeError: NoneType.get`、≥ ~84 日で HTTP 403 を返し、403 は Python プロセスが存続する限り後続呼び出しも汚染する。+49 日は 3550 行で正常動作を確認。そのためデフォルト窓を 45 日に短縮。**FMP 再検証が必要**: `provider="fmp"` は `FMP_API_KEY` が未設定のため keyless 検証パスでは実行できず、90 日窓が FMP で動作するかは未確認。design phase で FMP キー投入 + 再検証を予定 |
...
2. ティッカー横断で 1 ショット fetch（ticker 単位の重複呼び出しを避ける）：
   - `obb.equity.calendar.earnings(start=<TODAY>, end=<TODAY + N days>, provider="nasdaq")` with `N` defaulting to `45` (overridable via `--earnings-window-days`, bounded `[1, 90]`) → 入力ティッカー集合の直近・次回決算日。出力後に input ticker 集合で内部フィルタ。2026-04-30 の実 API 検証で nasdaq は +49 日までで正常動作、+56 日以降でサーバー側エラー、+84 日以降で 403 poisoning を確認。**FMP 再検証後に 90 日に緩和する余地あり**（該当記述は §Existing primitives 決算日行を参照）
...
- **FMP 再検証が必要な項目（2026-04-30 の keyless 検証パスで保留、FMP_API_KEY 投入後に design phase で消化）**:
  1. **決算カレンダー窓**: nasdaq は +49 日までで動作、+56 日以降エラー、+84 日以降 403 poisoning（Req 3 プロバイダ注記）。FMP では 90 日窓が clean に動くか未確認。FMP で通れば `--provider fmp` 指定時のみデフォルト 90 日に戻す余地あり
  2. **`quote.last_price` の欠損**: yfinance は債券 ETF（TLT、LQD）で `last_price=None` を返す（Req 4 プロバイダ注記）。FMP で `last_price` が埋まるなら Req 4.7 のフォールバックチェーンは `--provider yfinance` 限定にできる可能性あり
  3. **OpenBB 技術指標のカラム名**: 現時点の想定では `obb.technical.*` の出力形状はプロバイダ非依存（技術指標はローカル計算）だが、念のため FMP 由来の `history.results` でも `close_RSI_14` / `close_MACDh_12_26_9` / Clenow の string `factor` が変わらないか再確認
...
1. When `--provider` is omitted, the entry-timing scorer shall use `yfinance` for every OpenBB equity call (`quote`, `historical`, `clenow`, `rsi`, `macd`) and `nasdaq` for the earnings-calendar call.
2. Where the user passes `--provider fmp`, the entry-timing scorer shall route equity calls to `fmp` while keeping the earnings-calendar call on `nasdaq`.
...
**Provider-ceiling note (live-verified 2026-04-30, yfinance/nasdaq keyless defaults):** nasdaq's earnings-calendar endpoint returns `AttributeError: NoneType.get` at windows ≥ ~56 days and HTTP 403 at ≥ ~84 days; once the 403 fires it poisons every subsequent calendar call for the lifetime of the Python process (a fresh process is required to recover). The original 90-day window mandated by the pre-verification draft of this requirement is therefore not achievable against the keyless nasdaq provider. Defaults below are shortened to 45 days, which sits safely inside the working band (+49d returned 3550 rows) and still covers the next quarterly cycle for actively-reporting tickers — the only horizon the analyst layer acts on (the proximity threshold in Req 7.2 is 5 calendar days). **Pending FMP re-verification:** this ceiling was observed against `provider="nasdaq"` only; `provider="fmp"` requires `FMP_API_KEY` and could not be exercised in the keyless verification pass. Design phase should repeat the binary-search against `provider="fmp"` before accepting the 45-day cap as the universal default — if FMP lifts the ceiling cleanly at 90 days, Req 3.1 should be re-amended to keep 90 as the default under `--provider fmp` and 45 only under the default `--provider yfinance` (which routes calendar calls to nasdaq per Req 2.1).

#### Acceptance Criteria

1. When resolving upcoming earnings, the entry-timing scorer shall call `obb.equity.calendar.earnings(start=<TODAY>, end=<TODAY + N days>, provider="nasdaq")` exactly once per invocation regardless of ticker count, where `N` defaults to `45` to stay inside the live-verified nasdaq working window and is overridable via `--earnings-window-days`.
...
**OpenBB output-shape note (live-verified 2026-04-30 against yfinance-default):** the three `obb.technical.*` endpoints do not expose the clean field names the pre-verification draft assumed. `obb.technical.clenow` returns `factor` as a **string** (observed: `"0.63826"`), not a float; arithmetic on it raises without an explicit coercion step. `obb.technical.rsi` emits the output column as `close_RSI_14` (not `rsi`). `obb.technical.macd` emits `close_MACDh_12_26_9` / `close_MACDs_12_26_9` / `close_MACD_12_26_9` (not `histogram` / `signal` / `macd`). The suffix/substring-search pattern already used by `scripts/momentum.py::_last_with` and the `_to_float` coercion already used by `scripts/sector_score.py::fetch_clenow` are the authoritative precedents. These shape quirks are provider-level artifacts of the technical library OpenBB bundles, not of yfinance itself, so they apply identically regardless of which price provider feeds `obb.equity.price.historical`. **Pending FMP re-verification:** the mechanical shape is not expected to change when the upstream historical fetch is routed through FMP (the technical indicators are computed locally), but design phase should re-run the column-name diagnosis with `--provider fmp` so the assumption is verified rather than inherited.

**Quote-endpoint `last_price` note (live-verified 2026-04-30 against yfinance):** `obb.equity.price.quote(provider="yfinance")` returns `last_price=None` for bond ETFs (TLT and LQD observed; `prev_close`, `open`, `high`, `low`, `year_high`, `year_low`, `ma_200d`, `ma_50d` were all populated for the same calls). Without a fallback, `range_pct_52w` and `ma200_distance` silently collapse to `null` for every bond-ETF basket member. **Pending FMP re-verification:** this null-`last_price` behavior was observed against `provider="yfinance"` only; `provider="fmp"` requires `FMP_API_KEY` and could not be exercised in the keyless pass. If FMP populates `last_price` for bond ETFs cleanly, the fallback mandated below can be narrowed to "only when provider is yfinance". Until that verification is done, the fallback applies universally.

#### Acceptance Criteria

1. For every input ticker, the entry-timing scorer shall call `obb.equity.price.quote` once and extract `last_price`, `year_high`, `year_low`, `ma_200d`, `ma_50d`, `volume_average`, and `volume_average_10d`.
...
7. Where the provider reports `volume_average` (3-month rolling) or `volume_average_10d`, the entry-timing scorer shall forward those fields verbatim into a sibling `volume_reference` block labelled `{window: "3m_rolling"}` and `{window: "10d"}` respectively so the provider-native fields remain distinct from the locally computed 20-day window.
